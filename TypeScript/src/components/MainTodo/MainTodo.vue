<template>
  <div ref="warpper" class="warpper">
      <div  v-for="(item, index) in CompletedTask" :key="index">
        <div ref="uncompletedItems" v-if="!item.Iscompletd" class="Task uncompleted">
...
const uncompletedItems =ref<HTMLDivElement[]>([])
...
const checkTask = computed(() => {
  ...
  res.value = CompletedTask.value.some(item=>{
    if(item.Iscompletd){ return true }
  })
  return res.value
})
...
const UpdateElement=(status:boolean,index?:number):void =>{
    if(status){
      uncompletedItems.value.forEach((item  , index) => {
        if(index === CompletedTask.value.length -1){
